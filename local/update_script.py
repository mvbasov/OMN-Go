...
def update_application():
    ...
    # 1. Aggressive Global Version Catch-Up (Fixes the 1.0.42 stuck footer bug)
    ...
    for filepath in files_to_bump:
        ...
        if os.path.exists(actual_path):
            ...
            # Force update any lagging APP_VERSION variable regardless of what 1.0.x it is on
            content = re.sub(r'APP_VERSION\s*=\s*"1\.0\.\d+"', f'APP_VERSION = "{target_version}"', content)
            
            # Force update the hardcoded footer fallback we injected in 1.0.47
    ...
    # 3. Inject Clear Button into Console Modal
    ...
    if os.path.exists(html_path):
        ...
        # Patch A: Add the Clear Button to the Header HTML
        old_header = r"header.innerHTML = '<span>JS Console Output</span><button id=\"goomn-console-close\" style=\"background:#ff5555; color:white; border:none; border-radius:4px; padding:4px 12px; cursor:pointer;\">Close</button>';"
        new_header = r"header.innerHTML = '<span>JS Console Output</span><div><button id=\"goomn-console-clear\" style=\"background:#888; color:white; border:none; border-radius:4px; padding:4px 12px; cursor:pointer; margin-right:8px;\">Clear</button><button id=\"goomn-console-close\" style=\"background:#ff5555; color:white; border:none; border-radius:4px; padding:4px 12px; cursor:pointer;\">Close</button></div>';"
        
        if old_header in normalized_content:
            normalized_content = normalized_content.replace(old_header, new_header)

        # Patch B: Add the onclick logic to clear the arrays and UI
        old_handler = r'''                document.getElementById('goomn-console-close').onclick = () => {
                    consoleModal.style.display = 'none';
                };'''
        new_handler = r'''                document.getElementById('goomn-console-close').onclick = () => {
                    consoleModal.style.display = 'none';
                };
                document.getElementById('goomn-console-clear').onclick = () => {
                    logs = [];
        ...
        print(f"Successfully added Console Clear button to {html_path}")

    # 4. Output Standardized Git Commit Message
    commit_msg = """feat(frontend): add clear button to js console and force version sync
    
- Implement aggressive regex version synchronizer to rescue trailing `APP_VERSION` values.
- Add "Clear" button to JS Console popup to flush arrays and reset UI counters.
- Synchronize Version to 1.0.49.
